	...
	class TracPlugin extends TBGModule
	{
		...
		public function createProject($name)
		{
			...
			if (!TBGContext::getScope()->hasProjectsAvailable())
			{
				...
				throw new Exception($i18n->__("There are no more projects available in this instance"));
			}
			if (($p_name = $name) && trim($p_name) != '')
			{
				...
				{
					$this->getResponse()->setHttpStatus(400);
					throw new Exception($i18n->__('A project with the same key already exists'));
				}
				catch (Exception $e)
				{
					$this->getResponse()->setHttpStatus(400);
					throw new Exception($i18n->__('An error occurred: '. $e->getMessage()));
				}
			}
			$this->getResponse()->setHttpStatus(400);
			throw new Exception($i18n->__("You don't have access to add projects"));
		}
